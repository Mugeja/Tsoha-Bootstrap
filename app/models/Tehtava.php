...
class Tehtava extends BaseModel {
    ...
    public static function tulostaTehtavat($kayttaja_id, $status) {
        if ($status == 'fuksi') {
            $query = DB::connection()->prepare('SELECT T.*, K.suoritettu, K.kuvaus, K.hyväksyjä FROM Tehtävä T INNER JOIN Käyttäjän_tehtävät K on '
                    . 'T.ID = K.tehtävä_id '
                    . 'AND T.status = :status '
                    . 'AND K.käyttäjä_id = :kayttaja_id '
                    . 'ORDER BY T.nimi');
            $query->execute(array('kayttaja_id' => $kayttaja_id, 'status' => 'fuksi'));
            ...
            $query = DB::connection()->prepare('SELECT T.*, K.suoritettu, K.kuvaus, K.hyväksyjä FROM Tehtävä T INNER JOIN Tuutorien_tehtavat K on '
                    . 'T.ID = K.tehtävä_id '
                    . 'AND T.status = :status '
                    . 'AND K.käyttäjä_id = :kayttaja_id '
                    . 'ORDER BY T.nimi');
            $query->execute(array('kayttaja_id' => $kayttaja_id, 'status' => 'tuutori'));
            $rivit = $query->fetchAll();
            $tehtavat = array();
            foreach ($rivit as $rivi) {
                $tehtavat[] = new Tehtava(['id' => $rivi['id'],
            ...
            $query = DB::connection()->prepare('SELECT T.*, K.suoritettu, K.kuvaus, K.hyväksyjä FROM Tehtävä T LEFT JOIN Käyttäjän_tehtävät K on '
                    . 'T.ID = K.tehtävä_id '
                    . 'AND K.käyttäjä_id = :kayttaja_id '
                    . 'ORDER BY T.nimi');
